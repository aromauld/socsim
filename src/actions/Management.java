...
public class Management implements MenuInterface{
	...
	private void GetAvailableManagers() //Returns the list of all unemployed managers
	{
		...
		available_managers_player.add(new Player(1));
		available_managers_player.add(new Player(2));
	...
	private void GetTechnologists() //Returns the list of all technologists in the game
	{
		technologists = new java.awt.List();
		technologists.add("Player 2");
		technologists.add("Player 3");
		technologists.setBounds(210, 200, 150,230);
		sendJobOffer = Coex.btn("Send Job Offer", 210, 430, 150, 20, "send_job_offer"); sendJobOffer.setEnabled(false);
	}
	
	private void GetProjectList() //Returns the list of projects that the company has
	{
	...
	private void GetEmployees() //Returns the list of players that are working for the company
	{
		Employees = new java.awt.List();
		Employees.add("Player 4");
		Employees.add("Player 5");
	...
	private void SetBaseComp() //Sets up the base components
	{
		...
		{	
			...
			if(SimManager.player.GetStatus() == "CEO")
			{
				//Manager List
			...
			else if(SimManager.player.GetStatus() == "CTO")
			{
	...
	{
		//CEO
		if(available_managers.getSelectedIndex() >= 0) { hire_cfo.setEnabled(true); hire_cto.setEnabled(true); }
